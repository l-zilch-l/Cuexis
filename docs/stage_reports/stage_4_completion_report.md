...
权威计划：[Stage 4 实施计划](../stage_plans/stage_4_implementation_plan.md) §7.8、§12。
...
| 批次 | 实际交付与证据 |
| --- | --- |
| S4-A | STATIC `cuexis_animation`、typed input 头、compile 合同与诊断冻结；见 [S4-A](260824-stage-4-s4-a-module-wiring.md)。 |
| S4-B | owning compile、绝对 local Beat、fill/iterations、continuous/step 采样；见 [S4-B](260825-stage-4-s4-b-absolute-sampling.md)。 |
| S4-C | Override/Additive Layer 混合写入 PropertyWriteBuffer；见 [S4-C](260826-stage-4-s4-c-layer-mixing.md)。 |
| S4-D | 统一 PropertyResolver、OverrideToken、BasePropertyCommand 与 Runtime 编排；见 [S4-D](260827-stage-4-s4-d-property-resolver.md)。 |
| S4-E | Playback compile/own/commit 动画状态；失败 reload 不改 active；见 [S4-E](260827-stage-4-s4-e-playback-ownership.md)。 |
| S4-F | 默认 Session 接受合法非空动画；裁剪 Session 仍拒绝；跨 source identity 与 FrameDigest v3 一致；见 [S4-F](260827-stage-4-s4-f-consumer-capability.md)。 |
| S4-G | ChartLimits/600,000 write 上限、诊断 sentinel、warmed 零分配或有界合同、F4 风格探针；见 [S4-G](260827-stage-4-s4-g-safety-allocation.md)。 |
| S4-H | 本地 Debug/Release 门禁见 [S4-H 本地快照](260827-stage-4-s4-h-local-validation.md)；最终 SHA hosted 见下文。 |
...
[S4-H 本地快照](260827-stage-4-s4-h-local-validation.md) 记录 hosted 发布前的 MSVC Debug/Release
...
关闭后的下一阶段是 [Stage 5](../stage_plans/stage_5_implementation_plan.md)，状态为
