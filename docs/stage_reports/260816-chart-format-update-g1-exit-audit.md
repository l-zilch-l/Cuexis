...
| 2 | 规范、Schema、typed Reader、Writer/Canonicalizer 和 validator 一致 | PASS | [C1](260811-chart-format-update-c1-reader.md) 与 [C2](260811-chart-format-update-c2-lowering.md) 已关闭 Schema/Reader/Writer/lowering；生产 Schema 位于 `schemas/`，合同测试位于 `tests/json_support/` 与 `tests/chart/` |
| 3 | v1/v2/v3 历史读取与显式、非覆盖迁移 | PASS | [D1/D2](260813-chart-format-update-d-migration.md)、[D3](260814-chart-format-update-d3-equivalence.md) 和 [D close](260814-chart-format-update-d-close.md) 保留旧 Reader、默认 CLI v3、显式 `--target 4`、原子输出与运行时等价 |
...
| 5 | Playback prepare/activate/reload/seek 成功与失败 headless 证据 | PASS | [F1](260815-chart-format-update-f1-headless.md)、[F2](260815-chart-format-update-f2-package-consumers.md)、`tests/playback/playback_v4_prepare_tests.cpp` 与 `playback_migration_equivalence_tests.cpp` 覆盖事务成功、失败 reload、seek/stop 与 active-state 保持 |
| 6 | clean external consumer 只使用 public Playback headers | PASS | [F2](260815-chart-format-update-f2-package-consumers.md)、`tests/external/playback_consumer.cpp` 和 `cmake/VerifyExternalConsumer.cmake` 覆盖 static/shared、add_subdirectory/find_package 与安装头隔离 |
| 7 | round-trip、FrameDigest/identity、数组和实体顺序确定性 | PASS | [F3](260815-chart-format-update-f3-determinism.md) 与 `cmake/VerifyCfuF3Determinism.cmake` 固定 canonical CXC、migration、semantic identity、FrameDigest v3 和 diagnostics fingerprint |
| 8 | CXC metadata、entry order 和 package bytes 跨平台 golden | PASS | [F close](260816-chart-format-update-f-close.md) 记录 GCC、Clang、MSVC、MinGW 六份 artifact 的 CXC 与 fingerprint 完全一致 |
| 9 | 空动画 v4 通过；非空动画稳定拒绝 | PASS | [E2](260814-chart-format-update-e2-prepare.md) 与 F1/F2 consumer 证明静态/参数化 v4 成功，非空 Clip/CXT/Binding/Layer 以 `playback.capability.unsupported` 拒绝 |
| 10 | PlaybackSource/CXT/ParameterSet 所有权与 identity 边界 | PASS | [C3](260812-chart-format-update-c3-cxc.md)、[E1](260814-chart-format-update-e1-source.md)、[E3](260814-chart-format-update-e3-identity.md) 与 architecture tests 证明 archive 类型不进入 Playback 公共头、CXT 不伪装为 AssetId、参数不进入 source identity |
...
| 12 | E0 API、SDK compatibility、symbol/export 和 consumer | PASS | [E close](260814-chart-format-update-e-close.md) 关闭 API `0.6.0`、static/shared package、version rejection、export/import 与 Playback-only consumer |
| 13 | pack/validate/unpack 退出码与事务门禁 | PASS | [C4](260813-chart-format-update-c4-tools.md) 和 `cmake/VerifyCxcTools.cmake` 覆盖 exit `0/1/2`、atomic output、no-overwrite、staging cleanup、restore failure 与 binary round-trip |
| 14 | 仓库外旧资产清单或“未提供外部资产”决策 | PASS | [D close](260814-chart-format-update-d-close.md) 记录项目所有者于 2026-08-14 指示“未提供外部资产”，并保留全部兼容入口 |
