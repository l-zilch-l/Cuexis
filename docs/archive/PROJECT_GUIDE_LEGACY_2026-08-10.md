...
状态：historical snapshot；已由 `docs/PROJECT_GUIDE.md`、`docs/CURRENT_STATUS.md`、
`docs/architecture/` 和 `docs/ROADMAP.md` 拆分取代

归档日期：2026-08-10

本文保留整理前的完整项目指南，供历史追溯。当前产品、阶段和格式状态不得从本文推断。
...
CXC_FORMAT.md     CXC v1 与 Chart v4 的候选合同
CXT_FORMAT.md     CXT v1 JSON 模板、ChartParameter 与 Template Binding 合同
...
stage_plans/stage_chart_format_update_implementation_plan.md CXC/Chart 格式更新的决策、迁移与验收门禁
adr/0038-cxc-v1-and-chart-v4-boundary.md CXC v1、Chart v4 与 CXT v1 动画序列化提案
...
阶段 3 已最终关闭。Portable Presentation v1 公共值、资源闭包、Snapshot、digest、capability preflight、无 GPU Validation Sink、OpenGL adapter、Player 真实绘制和 Playback-only external package consumer 已实现并通过本地/hosted 验收。当前进入 Stage Chart Format Update：ADR 0038 提议 `.cxc` 严格交换包、`cuexis.chart` v4 动画序列化和包内 `.cxt` JSON 模板；CXT v1、播放前参数与 Template Binding 子决策已接受，ADR 0038 其余门禁仍待接受，之后才进入 Schema/Reader 实现。在此之前不把 CXC、CXT 或 Chart v4 描述为已支持。Cuexis Studio 的独立应用边界继续预留，但不阻塞 Playback SDK 外部消费。
...
```text
CFU-A 现有 v1/v2/v3 与外部资产盘点、用例 fixture
CFU-B CXC 身份、文档模型和时间语义 ADR
  CXT JSON、ChartParameter 与 Template Binding 子决策已接受
CFU-C Schema、typed Reader、Writer/Canonicalizer 和 validator
CFU-D v1/v2/v3 显式迁移与报告
CFU-E Playback prepare、capability、诊断和 reload 边界
CFU-F headless/external consumer、round-trip 和确定性 golden
CFU-G 跨平台验收与阶段 4 交接
```
...
前置条件：Stage Chart Format Update 的格式 ADR、Schema、迁移、prepare/preflight 和 external consumer 门禁全部关闭。
...
当前最高优先级是 Stage Chart Format Update：

```text
先完成 CXC/Chart 格式身份、载体、版本、时间域、引用、扩展和迁移的决策门禁
冻结 CXT JSON、ChartParameter、Template Binding 和 CXC 模板闭包规则
再实现 Schema、typed Reader/Writer、validator、迁移器和 Playback prepare 接入
为 Stage 4 交付 typed Clip/Binding/Property 输入，但不提前实现 AnimationSystem
保留 v1/v2/v3 的历史读取和显式迁移回归，不建立静默降级或长期双格式旁路
```
...
Stage Chart Format Update 的 CXC/CXT format ID、扩展名、容器布局和最终版本字段
完整商店发布、签名和分发流程
```
