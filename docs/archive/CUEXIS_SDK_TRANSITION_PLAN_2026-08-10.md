...
状态：historical snapshot；当前路线见 `docs/ROADMAP.md`，产品边界见 ADR 0027

归档日期：2026-08-10

本文保留整理前的完整 SDK transition plan，供历史追溯。当前阶段不得从本文推断。
...
CFU-B CXC 身份、Chart v4/CXT 文档模型、参数和时间语义 ADR
CFU-C Schema、typed Reader、Writer/Canonicalizer 和 validator
CFU-D v1/v2/v3 到新格式的显式迁移与报告
...
不执行运行时脚本、逐帧脚本回调或宿主任意代码
HostOverride 用于宿主临时覆盖可公开属性
StudioPreviewOverride 用于编辑预览
覆盖结束后仍按确定性基线恢复
```

### 12.8 阶段 5：材质、Shader 与宿主能力 Profile
...
### 12.9 阶段 6：Playback SDK 与独立 Player 产品化
...
### 12.14 阶段 10：可选内建 Vulkan Backend
...
### 12.16 阶段 12：稳定 ABI 与正式 Playback SDK v1
