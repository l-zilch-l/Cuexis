...
复跑完成，并修复 A19（headless 生成阻断）与 A20/A21（GCC `-Werror` release 构建阻断）；
首轮同 SHA hosted 暴露 A20 后已修复待重推复验，owner acceptance 未闭合，Stage 6 保持 future
...
| R5 | 进行中；本地回归矩阵（Debug/Release/shared/headless/MinGW headless/GCC Release `-Werror` 全绿）、A19/A20/A21 修复（实现 SHA `0e501a5`）、最终 SHA 容量复跑与关闭报告完成；首轮同 SHA hosted 已运行（MSVC 通过，MinGW `release` 与 Linux `GCC Release`/`GCC Shared Release` 因 A20 失败，修复待重推复验），owner acceptance 未闭合 | [R5 报告](../../../stage_reports/stages/chart-format-foundation/2026-09-17-r5-regression-and-handoff.md)、[容量数据 Debug](../../../stage_reports/stages/chart-format-foundation/2026-09-17-r5-capacity-data.json)、[容量数据 Release](../../../stage_reports/stages/chart-format-foundation/2026-09-17-r5-capacity-data-release.json)、6 个配置的全量 CTest 记录、hosted run `35253027313`/`35253027357`/`35253027366` | 同 SHA hosted 复验通过与 owner 接受齐备后才授权 Stage 6 |
