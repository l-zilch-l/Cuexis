...
采用与 Tempo Event 同构的 Behavior Event（格式决策见 `docs/adr/0034-chart-v3-tempo-and-behavior-events.md`），保留 TimingMap、Material/Visibility 表现和调试能力；BehaviorClip、局部 Beat 与循环是否进入阶段 2 由 2E 门禁决定。目标改为“表达 Cuexis 谱面表现”，不扩张为任意游戏脚本系统。
...
```text
阶段 2A.1 按 ADR 0035 移除 cuexis.chart.simple，不建立 Simple v2
