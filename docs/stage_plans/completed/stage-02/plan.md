...
状态：completed；本地与后续 hosted 跨平台门禁均已关闭

完成证据见[阶段 2 完成报告](../../../stage_reports/stages/stage-02/completion.md)。阶段 2 交接时仍缺少的
hosted Linux 门禁后来随 [Stage 3 最终验收](../../../stage_reports/stages/stage-03/completion.md) 一并关闭。
...
状态：completed。下述本地结果是 Stage 2 交接时的验收快照；当时缺少的 hosted Linux
GCC/Clang、sanitizer 和 package consumer 证据后来由 Stage 3 最终验收补齐。
...
Stage 2 交接时 hosted Linux CI 尚无 run URL，因此当时只关闭了 2G 本地门禁。该历史缺口后来随
[Stage 3 最终验收](../../../stage_reports/stages/stage-03/completion.md) 关闭，不再是当前开放门禁。
