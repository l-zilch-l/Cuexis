...
通用 `Curve<T>` 不作为阶段 2 的谱面层格式；阶段 2 通过新的 Behavior Event version 扩展连续属性、Hermite 进度、Material/Visibility/ParentBinding 边界和 BehaviorClip 组合，不静默改变 version 1 的采样结果。具体 v3 字段见 `docs/CHART_FORMAT.md`。
