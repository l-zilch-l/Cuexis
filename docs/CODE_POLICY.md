...
宿主回调默认不得重入同一 PlaybackSession。ContentProvider、日志 Sink、帧 Sink 和输入提交接口必须记录调用线程、所有权和可阻塞性；Audio Thread 不调用宿主文件、日志、渲染回调或触发判定结果查询。
...
Audio Thread      SDL 实时供数和 AudioClock 原子快照
Worker            文件 I/O、解码、导入、Shader 编译和 Prepared 数据
```

Worker 不访问 EnTT Registry、OpenGL/图形 Context、SDL Window 或实时 AudioStream。Render/Audio Thread 不读取可变 ChartDocument。

跨线程数据使用拥有关系清晰的消息、不可变快照或预分配队列。互斥锁不得进入音频实时路径。所有要求特定线程的公共函数在文档或命名类型中标注，Debug 构建检查线程断言。
